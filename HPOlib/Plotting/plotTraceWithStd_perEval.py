...
def plot_optimization_trace(trial_list, name_list, optimum=0, title="",
                            log=True, save="", y_max=0, y_min=0, scale_std=1, xmax=100, every=10):
...
def main(pkl_list, name_list, autofill, optimum=0, save="", title="", log=False,
         y_min=0, y_max=0, scale_std=1, cut=sys.maxint, xmax=100, every=10):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--xmax', dest='xmax', type=float,
                        default=100, help='Maximum of the x axis plot')
    ...
    main(pkl_list=pkl_list_main, name_list=name_list_main, autofill=args.autofill, optimum=args.optimum,
         save=args.save, title=args.title, log=args.log, y_min=args.min, y_max=args.max, scale_std=args.scale, xmax=args.xmax, every=args.every)

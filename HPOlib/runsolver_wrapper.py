...
def make_command(cfg, fold, param_string, run_instance_output):
    ...
    python_cmd += ' --data_path %s' % cfg.get('HPOLIB', 'data_path')    # add cmd arg to locate dataset
    python_cmd += ' --dataset %s' % cfg.get('HPOLIB', 'dataset')
    python_cmd += ' --use_caching %s' % cfg.get('HPOLIB', 'use_caching')
    python_cmd += ' --cv_folds %s' % cfg.get('HPOLIB', 'cv_folds')
